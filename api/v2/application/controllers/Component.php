...
class Component extends CI_Controller {
	...
	public function lists()
	{
		...
		$results = $this->component_model->get_lists();
		foreach($results as $result) {
			$result['component_name'] = ucwords(strtolower($result['component_name']));
			$new[] = $result;
		}
	...
	public function add()
	{
		...
		$result = $this->component_model->add($data);
	...
	{
		$this->login_model->checktoken();
		$result = $this->component_model->get_by_id($id);
		if($result == false){
			$return['success'] = false;
	...
	public function update()
	{
		...
		if($data){
			$result = $this->component_model->update(array('id' => $inputs['id']), $data);
			$return['success'] = true;
	...
	{
		$this->login_model->checktoken();
		$result = $this->component_model->delete($id);
		if($result == true){
			$return['success'] = true;
	...
	private function _validate()
	{
		...
		{
			$return['success'] = FALSE;
			$return['err']['component_type'] = 'Component type is required';
		}
		if($inputs['component_name'] == '')
		{
			$return['success'] = FALSE;
			$return['err']['component_name'] = 'Component name is required';
		}
